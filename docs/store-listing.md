...
| 3 | 撮影結果 | ✅ `screenshot-03.png` | 保存された縦長PNGを上下2分割して横に並べたもの。そのまま縮小すると本文が読めないため、この形で「1枚に収まった分量」を見せている |
...
>
> #3 の再作成が必要な場合は、ココソコで撮影した縦長PNGを用意し、上下2分割 → 各パネルを高さ800に縮小 → 16pxの隙間を空けて白背景の1280×800に中央配置する。
