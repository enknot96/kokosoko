...
| 項目 | 値 |
| --- | --- |
| ストアURL | https://chromewebstore.google.com/detail/hobllnakplgdhgkjdanilcgplbpanbcm |
| 拡張機能ID | `hobllnakplgdhgkjdanilcgplbpanbcm` |
| 公開バージョン | 1.0.0 |

> URLは拡張機能IDのみの短縮形。アイテム名入りの正式URLへ自動リダイレクトされるため、名前を変更してもリンクが切れない。`?authuser=` はログイン中のアカウント固有、`&hl=ja` は言語固定になるので、共有用URLには含めないこと。

### 更新するときは

掲載情報（スクリーンショット・プロモタイル・説明文など）だけの変更でも、ダッシュボードで更新して再度提出する必要がある。ただし**提出中も公開済みのバージョンはそのまま公開され続ける**ため、既存ユーザーにも新規インストールにも影響しない。
...
掲載情報はロケールごとに登録でき、その言語のユーザーには対応する掲載情報が、登録の無い言語のユーザーにはデフォルト言語のものが表示される。したがって日本語ロケールひとつでも、詳細な説明に日英両方を入れておけば英語圏のユーザーにも意味は通る。

より丁寧にするなら、公開後に英語ロケールを追加し、下記の日本語版・英語版の説明文をそれぞれ単独で登録して分ける（読みやすさが上がる）。
...
`manifest.json` の `homepage_url` と同じ値。

### 追加フィールドのその他の項目

| 項目 | 設定 | 理由 |
| --- | --- | --- |
| 公式 URL | **「なし」のまま** | 事前に所有権を確認したサイトから選ぶ欄。GitHubリポジトリは所有権確認ができないため選べない。任意項目 |
| 成人向けコンテンツ | **OFF** | 該当なし |
...
| スクリーンショット | 1280×800（推奨）または 640×400、角丸なし・余白なし、1〜5枚 | ✅ 4枚とも `assets/screenshots/screenshot-01〜04.png` に用意済み（すべて 1280×800） |
| 小プロモタイル | 440×280 | ✅ `assets/promo/promo-440x280.png`（1280×800版を縮小して作成） |
| マーキープロモタイル | 1400×560（任意） | — 作成しない（下記の理由） |
...
| 2 | 範囲選択中のオーバーレイ | ✅ `screenshot-02.png` | Wikipedia の Japan 記事上で、青い選択枠が表示されている状態 |
| 3 | 撮影結果 | ✅ `screenshot-03.png` | 保存された縦長PNGを上下2分割して横に並べたもの。そのまま縮小すると本文が読めないため、この形で「1枚に収まった分量」を見せている。**「2枚に分割保存される」と誤解されないよう、画像上部に青帯で注記を焼き込み済み** |
| 4 | ポップアップ | ✅ `screenshot-04.png` | ツールバーのアイコンとポップアップ（Full Page / Select Area） |

> 提出順は上記のとおり。1枚目のタイトルカードに続けて実画面（#2〜#4）が並ぶため、「実際のユーザー体験を示す」というポリシー要件を満たしている。
>
> #3 の再作成が必要な場合は、ココソコで撮影した縦長PNGを用意し、上部88pxに青帯（#1E6BE6）＋注記テキスト、残り712pxに上下2分割したパネルを16pxの隙間で中央配置する。ストアにはスクショごとのキャプション欄が無いため、**注記は画像内に焼き込む必要がある**。
...
1. ~~発行された拡張機能IDを控える~~ → `hobllnakplgdhgkjdanilcgplbpanbcm`
2. ~~`README.md` のバッジとインストールリンクを実URLに差し替える~~
3. ~~`README.md` のデモGIF・スクリーンショットを掲載する~~
4. ~~リポジトリの About 欄にストアURLを設定する~~

### 次回の更新でまとめて出すもの

掲載情報の変更は、変更のたびに再提出が必要になる。審査回数を減らすため、**次の提出時に以下をまとめて反映する**。

**1. 英語ロケールの追加**

Store listing タブで英語のロケールを追加し、本メモの以下をそれぞれ単独で登録する（日英併記をやめ、言語ごとに分ける）。

- アイテム名: `ココソコ - Koko Soko`（共通）
- 概要: 「概要（Summary）」の英語部分のみ
- 詳細な説明: 「詳細な説明 — English」をそのまま

あわせて日本語ロケール側からは英語文を外し、「詳細な説明 — 日本語」のみにする。

**2. マーキープロモタイル（1400×560）— 作成しない方針**

任意項目であり、Googleがストアの特集枠に掲載する際に使うもので、通常の検索結果や詳細ページには表示されないため。作るなら1400×560専用の横長構図でデザインし直す必要がある（既存のキービジュアルは比率1.6のため、中央クロップすると「Koko」の上と「Soko」の下が切れ、左右を背景で埋める方法も本体との境目が目立つ）。

### そのほかの任意タスク

- YouTube にデモ動画をアップロードし、Store listing の「YouTube 動画」欄に登録する（GIFはストアに掲載できない）
